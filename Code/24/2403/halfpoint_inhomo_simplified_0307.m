...
% C=1;
 ...
 % B=k2 maybe put it into cycle or set as a value
    L1=Ushalf./h;
    L2=-Ushalf./h;
    L3=ones(1,Numhalfcol).*0;
    L4=ones(1,Numhalfcol).*0;
    ...
    L11=FAhalf.*((Gamp/(F^2))./(2.*Ushalf)+(1/R).*(Hshalf.^(1/2)).*((B/cos(sc))^2)./(2.*(Ahalf.^2)));
    
    L12ori=Ushalf./Ahalf+(Gamp/(F^2))./(2.*Ushalf)+(1/R).*(Hshalf.^(1/2)).*((B/cos(sc))^2)./(2.*(Ahalf.^2));
    L12=FAhalf.*((Gamp/(F^2))./(2.*Ushalf)+(1/R).*(Hshalf.^(1/2)).*((B/cos(sc))^2)./(2.*(Ahalf.^2)));
...
[V,D]=eigs(LLHS,RRHS,200,'smallestabs');
